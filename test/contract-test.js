...
describe("Chained Vampires contract", function () {
    ...
    let Contract;
    let owner;
    let baseUri = "ipfs://QmZZ5vJ8f2TeMaicR7By4SJBWAVtj5cpoR8F6NsV5aotBz/"; // don't ever putting forget backslash at the end
    ...
    describe("Minting tests", function () {
        ...
        it("should display token URI", async() => {
            await Contract.summonVampire(1, { value: ethers.utils.parseEther("0.4") });
            let currId = await Contract.connect(owner).getCurrentTokenId();
            console.log(currId.toString());

            let ids = await Contract.walletOfOwner(owner.address);
            console.log(ids);

            let intID = parseInt(currId) - 1;
            console.log("Int ID: " + intID.toString());
            let uri = await Contract.connect(owner).tokenURI(ethers.BigNumber.from(intID.toString()));
